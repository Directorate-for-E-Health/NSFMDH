...
Importfil 1e-Variabelgruppe brukes til å beskrive variabelgrupper og prøvetyper med utgangspunkt i et sett standardiserte properties. Filen er basert på
...
|1 |Code |Obligatorisk |Unik navnestreng for en variabelgruppe/prøvetype. Store
...
Gruppen Prøvetyper defineres i henhold til MIABIS Detailed Samle Type (MIABIS-COLLECTION-14)

G_<Kortnavn>.SERUM
G_<Kortnavn>.WHOLE_BLOOD
etc.

For hver prøvetype defineres et basis-sett av prøvetypeattributter i klassen VARIABEL i henhold til MIABIS SAMPLE.
I Tillegg kan man ha lokale attributter som defineres av hver kilde

|2 |ParentCode |Obligatorisk |Forkortelse for kildenavn med prefiks G
for variabelGruppe/prøvetype og underscore, f.eks. G++_++HUNT. «ParentCode» for
toppnoden skal være lik kortnavnet på datakilden, f.eks. HUNT. +
Eller referere til overliggende variabelgruppe i et hierarki.

|3 |Hierarchy |Valgfri |Brukes ikke.

|4 |PreferredTerm |Obligatorisk |Kort beskrivende navn på den enkelte
variabelgruppe/prøvetype. PreferredTerm på toppnoden skal ha navn lik
...
variabelgruppene/prøvetypene i en egendefinert rekkefølge som f.eks. avviker fra en
...
Brukes for å angi fra når en variabelgruppe/prøvetype er gyldig.
...
en variabelgruppe/prøvetype. Krever en ny rad i importfilen dersom mer enn ett
synonym.
...
|12 |Beskrivelse |Obligatorisk |Utførlig beskrivelse av variabelgruppen/prøvetypen.
...
klassen «Filter».
Prøvetyper skal alltid ha verdien 1.
...
|16 |Gruppetype | Anbefalt |Definerer om gruppen er en gruppe variabler eller en Prøvetype
...
Hvis ikke gitt, behandles den som 1

|17-N |Mulige tilleggsproperties |Valgfri |Gir mulighet for å legge til
